...
function sort_rows() {
    ...
    for (var i = 0; i < num_choices; i++) {
        permut[i] = new Array(rows[i].getElementsByTagName("var")[0].innerHTML, 
		                      rows[i].getElementsByTagName("select")[0].selectedIndex,
							  rows[i].getElementsByTagName("td")[0].id);
    }
    ...
    for (var i = 0; i < permut.length; i++) {
		...
		rows[i].getElementsByTagName("td")[0].id = permut[i][2];
	}
    read_rows();
    set_row_borders();
}
