...
/**
			...
			if ($oevk <= 0) {
				...
				$this->redirect();
				 ...
				 if (szavazottMar($oevk, $user) & 
							(!isOevkSzavazas($oevk)) &
							(!isMiniszterElnokSzavazas($oevk)) &
							(!isOrszagosListaSzavazas($oevk)) 
						) {
  ...
  * @param string filter
  */ 	
	public function szavazatDelete($oevk, $user, $filter, $szavazatTable='') {
  ...
  * @param string filter
  */ 	
  public function szavazatDelete2($oevk, $user, $filter, $szavazatTable='') {
			global $evConfig;
    ...
    */ 	
    public function eredmeny($oevk, $user, $filter, $szavazatTable = '#__szavazatok') {
			...
			*/
			...
			if ((!isOevkSzavazas($oevk) & !isMiniszterElnokSzavazas($oevk) & !isOrszagosListaSzavazas($oevk) ) |
				($oevk == 116)) {
				echo '<p></p><h3>Válassza ki a szavazást!</h3><p></p>';
				return;
			}
    ...
    * @param filter
    */
  	public function tamogatas($jelolt, $user, $filter, $szavazatTable='') {
		...
		* kezdolap
		*/
		public function defaulttask($oevk, $user, $filter, $szavazatTable='') {
			if (!isOevkSzavazas($oevk)) {
				$oevk = oevkFromUser($user);
			}
			if (isOevkSzavazas($oevk)) {
		...
		*/
		public function szavazatedit($oevk, $user, $filter, $szavazatTable='') {
			global $evConfig;
		...
		*/
		public function szavazatok($oevk, $user=null, $filter='', $szavazatTable = '#__szavazatok') {
			...
			/**
			* jelölt adat beküldő képernyő
			* @JRequest id = OEVK kategoria id
			*/
			public function add() {
			}

			/** jelölt adat form feldolgozó
			* JRequest: catid, nev, fblink, keplink, szoveg, cimke1, cimke2,...
			* - content tárolás (published = 0)
			* - cimkemap tárolás
			* - tájékoztató levél adminoknak
			* - üzenet arról, hogy ellenörizni fogják
			*/ 
			public function jeloltSave() {
			}
