...
<?php 
  $db->setQuery('select * from #__categories where id='.$db->quote($this->item->catid));
  $this->category = $db->loadObject();
  $catLink = JURI::root().'component/content/category?id='.$this->category->id;
  if ($this->category->parent_id == 8)
    echo '<h2><a href="'.$catLink.'">'. $this->category->title.'</a></h2>';  
?>
